...
if not (Path("data/train_split.tsv").is_file() and Path("data/val_split.tsv").is_file()):
    ...
    x_train_orig, x_val_orig, y_train_orig, y_val_orig = train_test_split(sentences, labels, test_size=0.25)  # 按照 75: 25 比例划分 训练集和验证集；结果的 type 不变，仍为 pd.Series
    data_handle.save_tsv_data([x_train_orig, y_train_orig], 'data/train_split.tsv')
    data_handle.save_tsv_data([x_val_orig, y_val_orig], 'data/val_split.tsv')
else:
    ...
    x_val_orig, y_val_orig = x_val_orig[:3000], y_val_orig[:3000]
