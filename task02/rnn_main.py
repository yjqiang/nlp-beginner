...
USE_GLOVE = False
PATH = 'saved_model/rnn_with_glove' if USE_GLOVE else 'saved_model/rnn_no_glove'
...
def evaluate(model: nn.Module, dataloader: DataLoader, dataset: MyDataset) -> float:
    ...
    model.eval()
    with torch.no_grad():
        for x, y, seq_lengths in tqdm(dataloader, total=len(dataloader)):
            x = x.to(DEVICE)
            y = y.to(DEVICE)  # shape: (batch_size, )   y[i] 是 x[i] 的分类真值，且 0 <= y[i] < CLASS_NUM
            scores = model.get_scores(x, seq_lengths)  # shape: (batch_size, CLASS_NUM)
...
def train():
    ...
    x_val, y_val, seq_lengths_val = x_val[:x_val.shape[0] // 2], y_val[:y_val.shape[0] // 2], seq_lengths_val[:len(seq_lengths_val) // 2]  # task01 数据基础上，把 val 再劈一个 test，见 rnn_load_and_test.py
    ...
    # 训练
    model = Model(vocab=VOCAB, embedding_size=EMBEDDING_SIZE, hidden_size=HIDDEN_SIZE, num_layers=NUM_LAYERS, class_num=CLASS_NUM, dropout_p=DROPOUT_P, embedding=EMBEDDING)
    ...
    for epoch in range(EPOCH):
        ...
        model.train()
        total_loss = 0.0
        for iterator, (x, y, seq_lengths) in tqdm(enumerate(train_dataloader), total=len(train_dataloader)):

            optimizer.zero_grad()  # 梯度缓存清零
            x = x.to(device=DEVICE)
            y = y.to(device=DEVICE)  # shape: (batch_size, )   y[i] 是 x[i] 的分类真值，且 0 <= y[i] < CLASS_NUM
            loss = model(x, seq_lengths, y)  # 标量
            loss.backward()
            optimizer.step()
